...
Iterative methods begin with one or more initial approximations. These are then used to derive a better approximtion which is then used to derive and even better approximation. The process continue until a root is found within a given tolerance. [Fixed-point iteration](#Fixed-Point-Iteration), [Newton's Method](#Newton's-Method)
